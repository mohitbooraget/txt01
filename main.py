...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello **I am Text Downloader Bot**. I can download videos from **text** file one by one.**\n\nDeveloper** : उससे क्या होगा काम से काम रखो 😂 **\nLanguage** : Python**\nFramework** : Pyrogram\n\nSend **TXT** File {Name : Link}")
...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello Bruh **I am Downloader Bot**. I can download videos from **text** file one by one.**\n\nLanguage** : Python**\nFramework** : Pyrogram\n\nSend **TXT** File {Name : Link}")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello **I am jw Downloader Bot**. I can download videos from **text** file one by one.**\n\nLanguage** : Python**\nFramework** :Pyrogram\n\nSend **TXT** File {Name : Link}"
       ,reply_markup=keyboard)
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
